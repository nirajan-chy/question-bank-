...
import { admin, api } from "@/services/api";
import {
  queryKeys,
  useAdminResource,
  useAdminResourceMeta,
...
// `id` are auto-generated; subject metadata comes from the picker below.
const AUTO_FIELDS = new Set(["slug", "id", "subjectSlug"]);
// Resources whose records belong to a course → semester → subject hierarchy.
// The free-text fields are replaced by a cascading picker.
const SUBJECT_LINKED_RESOURCES = new Set([
  "past-papers",
  "notes",
  "question-banks",
  "mock-tests",
...
/**
 * Merge the cascading course/semester/subject selection into the record
 * payload, filling every subject metadata column the server expects.
 */
function applyPickToPayload(
  payload: Record<string, unknown>,
  picked: { courseSlug: string; semester: string; subjectSlug: string },
  subjects: Subject[],
  hasSemesterField: boolean
): { error?: string; values?: Record<string, unknown> } {
  if (!picked.subjectSlug) {
    return { error: "Pick the subject this file belongs to first" };
  }
  const subject = subjects.find((s) => s.slug === picked.subjectSlug);
  if (!subject) {
    return { error: "Selected subject could not be loaded — pick it again" };
  }
  return {
    values: {
      ...payload,
      ...
      level: subject.level,
      courseSlug: picked.courseSlug || null,
      ...(hasSemesterField && picked.semester ? { semester: Number(picked.semester) } : {}),
    },
  };
}
  ...
  const fields = useMemo(
    () =>
      (meta?.attributes ?? [])
        ...
        .filter((f) => !(SUBJECT_LINKED_RESOURCES.has(resource) && PICKER_MANAGED_FIELDS.has(f.key))),
    [meta, resource]
  );
...
function ResourceForm({
  ...
  const [picked, setPicked] = useState<{ courseSlug: string; semester: string; subjectSlug: string }>(
    () =>
      subjectLinked
        ? {
            courseSlug: String(initial?.courseSlug ?? ""),
            semester: initial?.semester != null ? String(initial.semester) : "",
            subjectSlug: String(initial?.subjectSlug ?? ""),
          }
        : EMPTY_PICK
  );
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (subjectLinked) {
      const merged = applyPickToPayload(payload, picked, subjects, resource === "past-papers");
      if (merged.error) {
