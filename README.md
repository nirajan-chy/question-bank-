...
- **Grounded Q&A** — the AI answers _only_ from the user's documents, with numbered citations (document + page) returned for verification
...
cd client
npm install
cp .env.example .env.local   # optional — NEXT_PUBLIC_BASE_URL defaults to the local API
...
| Variable               | Description                                                       |
| ---------------------- | ----------------------------------------------------------------- |
| `NEXT_PUBLIC_BASE_URL` | Base URL of the backend API (default `http://localhost:5000/api`) |
...
- **Client-server components** (`app/subjects/[slug]`, `app/universities/[slug]`, `app/classes/[slug]`, `app/blog`, `app/blog/[slug]`) use `client/data/*.json` at build time for `generateStaticParams` + metadata only, so `next build` works without the backend. All page _content_ is fetched from the API at runtime.
